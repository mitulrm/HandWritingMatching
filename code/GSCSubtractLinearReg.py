...
# Linear Regression Model for GSC ssubtracted features dataset
def GSCSubtractLinearReg(x_train, x_val, x_test, y_train, y_val, y_test, verbose, use_basis='N'):
    print('--------- GSC Subtract Linear Regression ---------')
    algo = 'linear'
    ...
    w = np.dot(np.random.rand(x_train.shape[1]),1)
    ...
    df.plot()
    plt.savefig('GSC_Linear_regression_Subtract.jpg',grid=True, figsize=(7,8))
