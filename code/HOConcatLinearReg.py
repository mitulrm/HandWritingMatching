...
def HOConcatLinearReg(x_train, x_val, x_test, y_train, y_val, y_test, verbose, use_basis='N'):
    print('--------- Human Observed Concatenate Linear Regression ---------')
    ...
    w = np.dot(np.random.rand(x_train.shape[1]),3)
    regularization = 0.05
    learning_rate = 0.04
    L_Erms_Val = []
    L_Erms_TR = []
    ...
    plot = df.plot()
    plt.savefig('HO_Concat_Linear.jpg',grid=True, figsize=(7,8))
