...
def GSCConcatNN(x_train, x_val, x_test, y_train, y_val, y_test, verbose):    
    print('--------- GSC Concatenate Neural Network ---------')    
    ...
    # Sequential Model of 5 hidden layers   
    model = Sequential()
    ...
    model.add(Dense(layer_5_nodes, input_dim=layer_4_nodes, kernel_regularizer=regularizers.l2(0),
                    activity_regularizer=regularizers.l1(0), kernel_initializer = initializers.RandomUniform(seed=123)))
    #model.add(Activation('relu'))
    model.add(LeakyReLU(alpha=0.3))
    model.add(Dropout(drop_out))
    
    model.add(Dense(output_nodes, input_dim=layer_5_nodes, kernel_regularizer=regularizers.l2(0),
    ...
    df.plot()
    plt.savefig('GSC_Concat_NN.jpg',grid=True, figsize=(7,8))
