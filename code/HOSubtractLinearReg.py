...
def HOSubtractLinearReg(x_train, x_val, x_test, y_train, y_val, y_test, verbose, use_basis='N'):
    print('--------- Human Observed Subtract Linear Regression ---------')
    algo = 'linear' # 'linear' or 'logistic'
    ...
    w = np.dot(np.random.rand(x_train.shape[1]),5)
    regularization = 0.1
    learning_rate = 0.1
    L_Erms_Val = []
    L_Erms_TR = []
    ...
    plot = df.plot()
    plt.savefig('HO_Subtract_Linear.jpg',grid=True, figsize=(7,8))