...
def HOSubtractLogisticReg(x_train, x_val, x_test, y_train, y_val, y_test, verbose):    
    print('--------- Human Observed Subtract Logistic Regression ---------')
    ...
    w = np.dot(np.random.rand(x_train.shape[1]),50)
    regularization = 0.1
    learning_rate = 0.05
    L_Erms_Val = []
    L_Erms_TR = []
    ...
    plot = df.plot()
    plt.savefig('HO_Subtract_Logistic.jpg',grid=True, figsize=(7,8))
